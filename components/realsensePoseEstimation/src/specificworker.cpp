...
void SpecificWorker::compute()
{
    ...
    {
        std::cerr << "I do not have odometry" << '\n';
    }
    ...
    for (const auto &[key, value] : cameras_dict) {
        ...
        if (odometry){
            qInfo () << "valor odometria" << v.x << v.y << v.z;
            cameras_dict[key].odometer->send_wheel_odometry(0,0,v);
            //std::cout<< cameras_dict[key].device_serial<<std::endl;
        }
        ...
        std::cout<<cameras_dict[key].device_serial<<pose_data.translation.x<<pose_data.translation.y<<pose_data.translation.z<<std::endl;
